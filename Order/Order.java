...
public class Order {
    ...
    public void placeOrder() {
        ...
        if (shippingAddress == null || items.isEmpty()) {
            throw new IllegalStateException("Order cannot be placed without a shipping address and items.");
        }

        // select payment method
        if (paymentMethod == null) {
            throw new IllegalStateException("Payment method must be selected before placing the order.");
        }
    ...
    public void closeOrder() {
        if (status != OrderStatus.DELIVERED) {
            throw new IllegalStateException("Order cannot be closed until it has been delivered.");
        }
